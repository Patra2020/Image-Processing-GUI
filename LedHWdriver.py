...
class LedHardware():
    """
    Models an HW connection
    """

    def __init__(self, baud_rate=9600, read_timeout=5):
        """
        Initializes the serial connection to the hardware board
        """
        ports = list(serial.tools.list_ports.comports())

        for portser in ports:
            comport = str (portser)
            serial_port = comport [0:4]
    ...
    def close(self):
        ...
        """
        self.conn.close()
        print ('Connection to hardware closed')

# abc = LedHardware()
# abc.digital_write( 5, 0)
# abc.digital_write( 6, 1)
# abc.digital_write( 7, 0)
# abc.close()